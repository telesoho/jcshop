...
<div class="ucenter container">
	...
	<div class="box m_10">
		<table class="border_table" width="100%" cellpadding="0" cellspacing="0">
			...
			<tr>
				<th>订单号：</th>
				<td><?php echo isset($this->data['order_no'])?$this->data['order_no']:"";?></td>
			</tr>

			<tr>
				<th>处理结果：</th>
				<td><?php echo Order_Class::refundmentText($this->data['pay_status']);?></td>
			</tr>

			<tr>
				<th>退款商品：</th>
				<td>
					<?php foreach($this->data['goods'] as $key => $item){?>
					<?php $goodsInfo = JSON::decode($item['goods_array'])?>
					<p>
						<a class="blue" href="<?php echo IUrl::creatUrl("/site/products/id/".$item['goods_id']."");?>" href="_blank">
							<?php echo isset($goodsInfo['name'])?$goodsInfo['name']:"";?> <?php if($goodsInfo['value']){?><?php echo isset($goodsInfo['value'])?$goodsInfo['value']:"";?><?php }?> X <?php echo isset($item['goods_nums'])?$item['goods_nums']:"";?>
						</a>
					</p>
					<?php }?>
				</td>
			...
			<?php if($this->data['seller_id']){?>
			<tr>
				<th>商家主页：</th>
				<td><a href="<?php echo IUrl::creatUrl("/site/home/id/".$this->data['seller_id']."");?>" target="_blank">进入主页</a></td>
			</tr>
			<?php }?>

			<tr>
				<th>申请时间：</th>
				<td><?php echo isset($this->data['time'])?$this->data['time']:"";?></td>
			</tr>

			<?php if($this->data['amount'] > 0){?>
			<tr>
				<th>退款金额：</th>
				<td class="red2 bold"><?php echo isset($this->data['amount'])?$this->data['amount']:"";?></td>
			</tr>
			<?php }?>

			<tr>
				<th>处理时间：</th>
				<td><?php echo $this->data['dispose_time']?$this->data['dispose_time']:'未处理';?></td>
			</tr>

			<tr>
				<th>处理建议：</th>
				<td><?php echo $this->data['dispose_idea']?$this->data['dispose_idea']:'未回复';?></td>
			</tr>

			<tr>
				<th>退款原因：</th>
				<td><?php echo isset($this->data['content'])?$this->data['content']:"";?></td>
			</tr>

			<tr>
				<th></th>
				<td>
					<label class="btn"><input type="button" value="返 回" onclick='event_link("<?php echo IUrl::creatUrl("/ucenter/refunds");?>")'></label>
				</td>
			</tr>
		</table>
	</div>
