...
//		$this->data_goods = $data_goods;
	...
	function recommender_login(){
		...
		if(!empty($recommender)){
			$this-$this->redirect('recommender_shop');
		}
...
//        $temp = '( user_id = ' . $this->user['user_id'];
	...
	function your_name(){
		...
		if($name){
			$this->img = IWeb::$app->config['image_host'].'/site/your_name_pic/name/'.$name.'/sex/'.$sex;
		}
		$this->redirect('your_name');
	}

	function your_name_img(){
		$this->redirect('your_name_img');
	}

	function your_name_pic(){
		$name = IFilter::act(IReq::get('name'), 'string');
		$sex  = IFilter::act(IReq::get('sex'), 'string');
		preg_match_all('/./u', $name, $names);
		header("Content-type: image/png");    //浏览器输出，如不需要可去掉此行
		switch(strlen($name)){
			case 6:
		...
		imagettftext($im, 22, 0, 271, 835, $white, $font, $sex);
		imagepng($im);
		imagedestroy($im);
	}
